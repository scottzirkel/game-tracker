...
export default function RosterManager({ side, faction }: RosterManagerProps) {
  ...
  const addModel = (unitInstance: UnitInstance) => {
    const unit = mod.getUnitById(unitInstance.unitId) as GenericUnit | undefined;
    if (!unit || unitInstance.currentModels >= unit.modelsPerUnit.max) return;
    
    updateUnit(unitInstance.id, { 
      currentModels: unitInstance.currentModels + 1,
      totalModels: Math.max(unitInstance.totalModels, unitInstance.currentModels + 1)
    });
  };
  ...
  const changeTotalModels = (unitInstance: UnitInstance, delta: number) => {
    ...
    const newTotal = Math.max(unit.modelsPerUnit.min, Math.min(unit.modelsPerUnit.max, unitInstance.totalModels + delta));
  ...
  const sortedUnits = [...(mod.UNITS as GenericUnit[])].sort((a, b) => a.name.localeCompare(b.name));

  const getBgColor = () => {
    switch (factionKey) {
      case 'Ultramarines': return 'rgba(59, 130, 246, 0.03)';
      case 'Adeptus Custodes': return 'rgba(251, 191, 36, 0.03)';  
      case 'Chaos Marines': return 'rgba(239, 68, 68, 0.03)';
      case 'Necrons': return 'rgba(16, 185, 129, 0.03)';
      case 'Tyranids': return 'rgba(168, 85, 247, 0.03)';
      default: return 'rgba(23, 23, 23, 0.5)';
    }
  };

  return (
    <Card className={`w-full relative overflow-hidden ${theme.plateBorder}`} style={{backgroundColor: getBgColor()}}>
      <CardContent className="p-4 relative pb-6">
        <div className="mb-6">
          {/* Add Unit Section */}
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
            {/* Left Side: Unit & Custom Name */}
            <div className="space-y-4">
              <div>
                <Label className="text-sm font-bold tracking-[0.12em] text-gray-200 font-rajdhani uppercase">Unit</Label>
                <Select value={selectedUnitId} onValueChange={(unitId) => {
                  setSelectedUnitId(unitId);
                  const unit = mod.getUnitById(unitId) as GenericUnit | undefined;
                  if (unit) {
                    setSelectedModelCount(unit.modelsPerUnit.min);
                  }
                }}>
              ...
              <div>
                <Label className="text-sm font-bold tracking-[0.12em] text-gray-200 font-rajdhani uppercase">Custom Name</Label>
                <Input
                  value={newUnitName}
            ...
            {/* Right Side: Add Button & Model Count */}
            <div className="space-y-4 flex flex-col items-end">
              <div>
                <Button onClick={addUnit} disabled={!selectedUnitId} className="w-32">
              ...
              <div className="text-right">
                <Label className="text-sm font-bold tracking-[0.12em] text-gray-200 font-rajdhani uppercase">Model Count</Label>
                <div className="flex justify-end">
                  <Select 
                    value={selectedModelCount.toString()} 
                    onValueChange={(count) => setSelectedModelCount(parseInt(count))}
                  ...
                  >
                    ...
                    <SelectContent>
                      {selectedUnitId && (() => {
                        ...
                        if (!unit) return null;
                        const options = [];
                        for (let i = unit.modelsPerUnit.min; i <= unit.modelsPerUnit.max; i++) {
                          options.push(
          ...
          {/* Mini Summary */}
          <div className={`flex items-center justify-between mb-4 p-3 bg-gradient-to-r ${theme.primaryFrom} ${theme.primaryTo} border ${theme.primaryBorder} rounded`}>
            <div className={`text-sm font-rajdhani tracking-[0.12em] uppercase ${theme.primaryMutedText}`}>
              Total Units: {units.length}
            </div>
            <div className={`text-lg font-orbitron font-bold ${theme.primaryText}`}>
        ...
        <div className="space-y-4">
          {units.length === 0 ? (
            ...
            units.map(unitInstance => {
              ...
              const points = mod.calculateUnitCost(unit as any, unitInstance.totalModels);
                ...
                <Card 
                  ...
                  className={`border transition-all duration-200 ${
                    unitInstance.isDead 
                      ...
                      : `${theme.primaryBorder} bg-gradient-to-r ${theme.primaryFrom} ${theme.primaryTo}`
                  }`}
                >
                  <CardContent className="p-4">
                    <div className="flex items-center justify-between mb-3">
                      <div className="flex items-center gap-3">
                        <h4 className={`font-bold font-rajdhani tracking-wide ${theme.primaryText}`}>
                          {unitInstance.name}
                        </h4>
                      ...
                      <div className="flex items-center gap-2">
                        <span className={`text-sm font-orbitron font-bold ${theme.primaryText}`}>
                          {points}pts
                        </span>
                        {unitInstance.isDead && (
                          <Button
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-5 gap-4 mb-3">
                      {/* Active Models */}
                      <div>
                        <Label className="text-xs text-gray-400 font-rajdhani uppercase">Active</Label>
                        <div className="flex items-center gap-2 mt-1">
                          ...
                          <span className={`font-orbitron font-bold min-w-[30px] text-center ${theme.primaryText}`}>
                          ...
                          <Button
                            ...
                            disabled={unitInstance.currentModels >= unitInstance.totalModels}
                            className="size-8 p-0"
                          >
                      ...
                      {/* Total Models */}
                      <div>
                        <Label className="text-xs font-rajdhani uppercase text-amber-300">Total</Label>
                        <div className="flex items-center gap-2 mt-1">
                          <Button
                            size="sm"
                            variant="outline"
                            onClick={() => changeTotalModels(unitInstance, -1)}
                            disabled={unitInstance.totalModels <= unit.modelsPerUnit.min}
                          ...
                          <Button
                            ...
                            variant="outline"
                            onClick={() => changeTotalModels(unitInstance, +1)}
                            disabled={unitInstance.totalModels >= unit.modelsPerUnit.max}
                            className="size-8 p-0 text-amber-300 border-amber-400/50"
                          >
                            <Plus className="w-3 h-3" />
                          </Button>
                      ...
                      {/* Wounds */}
                      <div>
                        <Label className="text-xs text-gray-400 font-rajdhani uppercase">Wounds</Label>
                        <div className="flex items-center gap-2 mt-1">
                          <Button
                            size="sm"
                            variant="outline"
                          ...
                          <span className={`font-orbitron font-bold min-w-[40px] text-center ${
                      ...
                      {/* Save */}
                      <div>
                        <Label className="text-xs text-gray-400 font-rajdhani uppercase">Save</Label>
                        <div className="flex items-center gap-1 mt-1">
                          <Shield className="w-4 h-4 text-blue-400" />
                          <span className={`font-orbitron text-sm ${theme.primaryText}`}>
                            {unit.save}+
                          </span>
                          {unit.invulnSave && (
                      ...
                      {/* Toughness */}
                      <div>
                        <Label className="text-xs text-gray-400 font-rajdhani uppercase">Toughness</Label>
                        <div className={`font-orbitron text-sm mt-1 ${theme.primaryText}`}>
                          T{unit.toughness}
                        </div>
                      </div>
                    </div>
                    ...
                    {unit.abilities && unit.abilities.length > 0 && (
                      <div className="mt-3">
                        <Label className="text-xs text-gray-400 font-rajdhani uppercase">Abilities</Label>
      ...
      <div className={`absolute bottom-0 left-0 right-0 h-2 ${theme.accentHazard} [background-image:repeating-linear-gradient(45deg,#f59e0b_0_12px,#111827_12px_24px)]`} />
    </Card>
  );
}
